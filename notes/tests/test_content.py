...
class TestContent(BaseClassData):
    ...
    def test_note_not_in_list_for_another_user(self):
        ...
        notes_on_page = response.context["object_list"]
        assert self.notes not in notes_on_page
    ...
    def test_note_in_list_for_author(self):
        ...
        assert self.notes in notes_on_page
        ...
        for urls in self.url_add_edit:
            with self.subTest(urls):
                ...
                self.assertIn("form", response.context,
                              f'На страницу {urls} не передается форма')
                self.assertIsInstance(
                    response.context["form"], NoteForm,
                    f'На страницу {urls} передается неверная форма')
